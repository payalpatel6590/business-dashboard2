...
exports.createRevenue = exports.createSale = exports.getChartData = exports.getRevenueData = exports.getSalesData = exports.getDashboardStats = void 0;
...
const getDashboardStats = async (req, res) => {
    try {
        ...
        const [currentSales, currentRevenue, currentUsers] = await Promise.all([
            ...
            Revenue_1.default.aggregate([
                { $match: { ...userId, date: { $gte: thisMonth } } },
                { $group: { _id: null, total: { $sum: '$amount' }, count: { $sum: 1 } } }
            ]),
            req.user.role === 'admin' ?
                User_1.default.countDocuments({ createdAt: { $gte: thisMonth } }) :
                Promise.resolve(0)
        ]);
        ...
        const [lastMonthSales, lastMonthRevenue, lastMonthUsers] = await Promise.all([
            ...
            req.user.role === 'admin' ?
                User_1.default.countDocuments({ createdAt: { $gte: lastMonth, $lt: thisMonth } }) :
                Promise.resolve(0)
        ]);
        // Calculate totals
        const totalSales = currentSales[0]?.total || 0;
        const totalRevenue = currentRevenue[0]?.total || 0;
        const totalUsers = req.user.role === 'admin' ? await User_1.default.countDocuments() : 0;
...
const getSalesData = async (req, res) => {
    try {
        ...
        const sales = await Sale_1.default.find(matchQuery)
            .sort({ date: -1 })
...
const getRevenueData = async (req, res) => {
    try {
        ...
        const revenue = await Revenue_1.default.find(matchQuery)
            .sort({ date: -1 })
            .populate('userId', 'name email');
        res.status(200).json({
            success: true,
...
const getChartData = async (req, res) => {
    try {
        ...
        let data;
        if (type === 'sales') {
            data = await Sale_1.default.aggregate([
                { $match: matchQuery },
                {
                    $group: {
            ...
            data = await Revenue_1.default.aggregate([
                { $match: matchQuery },
                {
                    $group: {
                        _id: { $dateToString: { format: dateFormat, date: '$date' } },
...
const createSale = async (req, res) => {
    try {
        ...
        const sale = await Sale_1.default.create(saleData);
...
const createRevenue = async (req, res) => {
    try {
        ...
        const revenue = await Revenue_1.default.create(revenueData);
